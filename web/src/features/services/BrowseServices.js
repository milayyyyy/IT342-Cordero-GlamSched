...
const CATEGORIES = ['All', 'Hair', 'Makeup', 'Nails', 'Skincare', 'Lashes', 'Brows', 'Waxing', 'Massage', 'Other'];
...
function BrowseServices() {
  ...
  return (
    <div className="services-container">
      ...
      {/* Category filter */}
      <div className="bs-category-filter">
        {CATEGORIES.map(cat => (
          <button
            key={cat}
            className={`bs-cat-btn${activeCategory === cat ? ' active' : ''}`}
      ...
      {filtered.length === 0 ? (
        <div className="empty-state">
          <p>{activeCategory === 'All' ? 'No services available yet' : `No ${activeCategory} services yet`}</p>
        </div>
      ) : (
        <div className="services-grid">
          {filtered.map(service => (
            ...
            >
              ...
              {service.category && (
                <span className="bs-cat-badge">{service.category}</span>
              )}
